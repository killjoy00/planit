...
export async function AcquisitionFunnel() {
  const [rows, fastJoinParticipants] = await Promise.all([
    db.$queryRaw<AcquisitionRow[]>`
      ...
      select
        ...
        count(fp.id) filter (where fp."createdAt" <= now() - interval '30 days') as matured_first_polls,
        count(fp.id) filter (
          where fp."createdAt" <= now() - interval '30 days'
            and exists (
      ...
      group by 1, 2, 3
      order by signups desc, source asc, campaign asc nulls last, use_case asc nulls last
    `,
  ...
  return (
    <section className="space-y-3">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((row) => {
              ...
              const matured = Number(row.matured_first_polls)
              const second = Number(row.second_poll_30d)
              return (
                <tr key={`${row.source}:${row.campaign ?? "none"}:${row.use_case ?? "none"}`} className="border-b border-gray-100 last:border-0">
                  <td className="px-4 py-2 font-medium text-gray-900">{row.source}</td>
                  <td className="px-4 py-2 text-gray-500">{row.campaign ?? "—"}</td>
                  ...
                  <td className="px-4 py-2 text-right tabular-nums text-gray-700">{second} {percent(second, matured) !== null ? `(${percent(second, matured)}%)` : "—"}</td>
                </tr>
              )
            })}
          </tbody>
      ...
      <p className="text-xs text-gray-400">The 30-day repeat rate only uses first polls old enough to have had the full window. Fast Join participants so far: {fastJoinParticipants}. Legacy accounts remain “legacy / unknown” instead of being backfilled with guesses.</p>
    </section>
  )
}
